...
/** PATCH /taxonomy/subtopics/:slug */
export function validatePatchSubtopic(req, res, next) {
  ...
  if (req.body.topicId !== undefined) {
    if (req.body.topicId === null) {
      return res.status(400).json({
        success: false,
        message: 'Field "topicId" cannot be null.',
      });
    }
    const id = String(req.body.topicId).trim();
